...
import { Trophy, Medal, Star, ChevronUp, ChevronDown, UserCircle2, Info } from 'lucide-react';
import { Link } from 'react-router-dom';
import { useLanguage } from '../contexts/LanguageContext';

const LeaderboardItem = ({ username, userId, rank, points, badges, level, email }) => {
  const { t } = useLanguage();
  ...
  const badgeProgressionInfo = {
    'Rookie Uploader': { threshold: 5, icon: <Star className="w-4 h-4 text-blue-500" /> },
    'Resource Master': { threshold: 20, icon: <Medal className="w-4 h-4 text-yellow-500" /> },
    'Knowledge Sharer': { threshold: 10, icon: <Trophy className="w-4 h-4 text-green-500" /> },
    'Helpful Reviewer': { threshold: 5, icon: <Star className="w-4 h-4 text-purple-500" /> }
  };
      ...
      <div className="flex items-center space-x-4 w-full sm:w-auto">
        ...
        <div className="flex-grow text-center sm:text-left flex items-center space-x-3">
          ...
          >
            ...
            <h3 className="font-semibold text-gray-800 dark:text-gray-200 group-hover:text-indigo-600 dark:group-hover:text-indigo-400">{username}</h3>
          </Link>
          <div 
      ...
      <div className="flex items-center space-x-2 mt-2 sm:mt-0">
        <div className="text-sm text-gray-600 dark:text-gray-400 text-center sm:text-right flex items-center space-x-2">
          <div className="flex items-center">
            <span>{t('leaderboard.columns.level')} {level}</span>
            <Info 
          ...
          <span>•</span>
          <span>{points} {t('leaderboard.columns.points')}</span>
        ...
        {rank <= 3 && (
          <div className="hidden sm:block">
            ...
            {rank === 2 && <Medal className="w-5 h-5 text-gray-400 dark:text-gray-500" />}
            {rank === 3 && <Medal className="w-5 h-5 text-yellow-700 dark:text-yellow-600" />}
          </div>
        )}
      </div>
        ...
        <div className="absolute z-10 top-full left-0 mt-2 w-full bg-white dark:bg-gray-700 border dark:border-gray-600 rounded-lg shadow-lg p-4">
          ...
          {Object.entries(badgeProgressionInfo).map(([badge, info]) => (
            <div key={badge} className="flex items-center justify-between mb-2">
              {info.icon}
              <span className="ml-2 flex-grow text-gray-800 dark:text-gray-200">{t(`leaderboard.badges.${badge}`)}</span>
              <div className="w-1/3 bg-gray-200 dark:bg-gray-600 rounded-full h-2.5">
                <div 
                  className="bg-blue-600 dark:bg-blue-500 h-2.5 rounded-full" 
                  style={{ width: `${Math.min((points / info.threshold) * 100, 100)}%` }}
                ></div>
              </div>
            </div>
          ))}
...
export default function Leaderboard() {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        const response = await axios.get('http://localhost:4000/api/contributions/leaderboard', {
          params: { timeFilter }
        });
