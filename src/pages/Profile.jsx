...
import { 
  Trophy, Medal, Star, BookOpen, Upload, Edit, 
  CheckCircle2, XCircle, Clock, User as UserIcon 
} from 'lucide-react';
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 hover:shadow-lg transition-all">
      <div className="flex items-center mb-3">
        {icon}
        <h3 className="ml-3 font-semibold text-gray-800 dark:text-gray-200">{badge}</h3>
      </div>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 flex items-start space-x-4 hover:shadow-lg transition-all">
      ...
      <div className="flex-grow">
        <h3 className="font-semibold text-gray-800 dark:text-gray-200">{resource.title}</h3>
        <p className="text-sm text-gray-600 dark:text-gray-400">{resource.description}</p>
        <div className="mt-2 flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
          <span>{t('profile.uploadDate')}: {new Date(resource.createdAt).toLocaleDateString()}</span>
      ...
      <div className="flex flex-col items-end">
        <div className="flex items-center space-x-1 text-sm">
          ...
          <span className="text-gray-800 dark:text-gray-200">{resource.rating || 'N/A'}</span>
        </div>
            ...
            <div>
              ...
              <div className="mt-2 flex items-center space-x-4 text-white">
                <div className="flex items-center">
                  ...
                  <span>{t('profile.level')} {profile.user.level}</span>
                </div>
                <div className="flex items-center">
                  <Star className="w-5 h-5 mr-2" />
                  <span>{profile.user.totalPoints} {t('profile.points')}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="p-6 dark:bg-gray-900">
          ...
          {activeTab === 'badges' && (
            <div className="grid md:grid-cols-2 gap-4">
              {Object.entries(profile.badgeProgression).map(([badge, details]) => (
                <BadgeCard
                  key={badge}
                  badge={badge}
                ...
                profile.contributions.map(contribution => (
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      {contribution.type === 'upload' && <Upload className="w-6 h-6 text-blue-500 dark:text-blue-400" />}
                      {contribution.type === 'review' && <Star className="w-6 h-6 text-yellow-500 dark:text-yellow-400" />}
                      <div>
